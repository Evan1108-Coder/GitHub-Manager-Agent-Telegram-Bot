...
//
// Design (mirrors the TrendForge self-update, adapted for this agent):
...
//  - DATA IS PRESERVED: all user state (the SQLite db under data/ and .env) is
//    gitignored, so git pull/reset never touch it. dataFilesProtected() verifies
...
// User-state files that must survive any update. All are gitignored (the whole
// data/ dir plus .env), so git can never touch them.
const DATA_FILES = ['data/agent.sqlite', '.env'];
...
// can route those through the friendly error explainer.
function checkForUpdate(deps = {}) {
  const run = makeRunner(deps);
  if (!isGitRepo(deps)) {
    const err = new Error('This bot is not running from a git checkout, so it cannot self-update.');
    err.kind = 'not_git';
    throw err;
  }
...
function healthCheck(deps = {}) {
  ...
  const run = makeRunner(deps);
  const probe =
    "require('./src/config');require('./src/db');require('./src/setup');" +
    "require('./src/scheduler');require('./src/agent');require('./src/planner');" +
    "require('./src/security');require('./src/files');require('./src/llm/providers');" +
    "require('./src/bot').createBot('0:healthcheck');require('./src/update');" +
    "console.log('HC_OK');";
  try {
...
function formatUpdateNotice(info) {
  ...
  const lines = ['🆕 A GitHub Manager Agent update is available — ' + verPart + '.'];
  ...
  lines.push('');
  lines.push('Run /update to install it. Your settings, jobs, history and .env are kept — only the code changes, and I roll back automatically if the new version fails to start.');
  return lines.join('\n');
}
