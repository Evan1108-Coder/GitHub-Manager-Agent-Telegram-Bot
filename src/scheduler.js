...
function createScheduledJob({ name, goal, schedule, plan, permissions = {}, outputStyle = 'concise', enabled = true }) {
  const nextRunAt = schedule.nextRunAt || computeNextRun(schedule);
...
function seedDefaultJobs() {
  ...
  const defaults = [
    ...
    },
    {
      ...
      schedule: { type: 'daily', hour: 22, minute: 30 },
...
async function runJob(job, bot, executor, now = new Date()) {
  ...
  try {
    ...
    const nextRunAt = schedule.type === 'once' ? null : computeNextRun(schedule, now);
