...
test('dataFilesProtected: the sqlite db and .env are gitignored', () => {
  ...
  assert.ok(dp.protected.includes('data/agent.sqlite'));
});

test('dataFilesProtected: flags a data file that is NOT ignored', () => {
  const { run } = fakeRunner({ unignored: ['data/agent.sqlite'] });
  const dp = upd.dataFilesProtected({ run });
  assert.equal(dp.allProtected, false);
  assert.ok(dp.unprotected.includes('data/agent.sqlite'));
});
...
test('compareDataSnapshots: identical signatures → ok, no mismatches', () => {
  const before = { '.env': { exists: true, size: 100, lines: 10 }, 'data/agent.sqlite': { exists: true, size: 50, lines: 5 } };
  const after = { '.env': { exists: true, size: 100, lines: 10 }, 'data/agent.sqlite': { exists: true, size: 50, lines: 5 } };
...
test('compareDataSnapshots: a changed byte size is flagged as a mismatch', () => {
  const before = { 'data/agent.sqlite': { exists: true, size: 100, lines: 10 } };
  const after = { 'data/agent.sqlite': { exists: true, size: 220, lines: 14 } };
  const di = upd.compareDataSnapshots(before, after);
  assert.equal(di.ok, false);
  assert.equal(di.mismatches[0].file, 'data/agent.sqlite');
});
